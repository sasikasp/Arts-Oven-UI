---
layout: ../../layouts/docLayout.astro
title: Badge
description: Badge Component Documentation
---

import Card from '../../components/Card.astro'
import TabItem from '../../components/TabItem.astro'
import Tabs from '../../components/Tabs.astro'

The Breadcrumb component in the Arts Oven UI Component Library is designed to provide a navigational aid that helps users understand their current location within a website's hierarchy. Breadcrumbs enhance usability by allowing users to navigate back to previous levels easily.

### Breadcrumb

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="flex justify-center">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="#">Home</a>
                        </li>
                        <li class="breadcrumb-item">
                            <a href="#">Library</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">
                            <a href="#">Data</a>
                        </li>
                    </ol>
                </nav>
            </div>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item">
                    <a href="#">Home</a>
                </li>
                <li class="breadcrumb-item">
                    <a href="#">Library</a>
                </li>
                <li class="breadcrumb-item active" aria-current="page">
                    <a href="#">Data</a>
                </li>
            </ol>
        </nav>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <nav aria-label="breadcrumb">
            <ol className="breadcrumb">
                <li className="breadcrumb-item">
                    <a href="#">Home</a>
                </li>
                <li className="breadcrumb-item">
                    <a href="#">Library</a>
                </li>
                <li className="breadcrumb-item active" aria-current="page">
                    <a href="#">Data</a>
                </li>
            </ol>
        </nav>
        ````
    </TabItem>
</Tabs>

### Key Elements

- `<nav>` Element: Wraps the breadcrumb for semantic navigation and accessibility.
- `<ol>` Element: Represents an ordered list of breadcrumb items.
- `<li>` Elements: Each list item represents a step in the breadcrumb trail.
- Active Item: The last item in the breadcrumb trail should have the `aria-current="page"` attribute to indicate it is the current page.

### Arrow Divided

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="flex justify-center">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb divide-arrow">
                        <li class="breadcrumb-item">
                            <a href="#">Home</a>
                        </li>
                        <li class="breadcrumb-item">
                            <a href="#">Library</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">
                            <a href="#">Data</a>
                        </li>
                    </ol>
                </nav>
            </div>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb divide-arrow">
                <li class="breadcrumb-item">
                    <a href="#">Home</a>
                </li>
                <li class="breadcrumb-item">
                    <a href="#">Library</a>
                </li>
                <li class="breadcrumb-item active" aria-current="page">
                    <a href="#">Data</a>
                </li>
            </ol>
        </nav>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <nav aria-label="breadcrumb">
            <ol className="breadcrumb divide-arrow">
                <li className="breadcrumb-item">
                    <a href="#">Home</a>
                </li>
                <li className="breadcrumb-item">
                    <a href="#">Library</a>
                </li>
                <li className="breadcrumb-item active" aria-current="page">
                    <a href="#">Data</a>
                </li>
            </ol>
        </nav>
        ````
    </TabItem>
</Tabs>

### Arrow Dot

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="flex justify-center">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb divide-dot">
                        <li class="breadcrumb-item">
                            <a href="#">Home</a>
                        </li>
                        <li class="breadcrumb-item">
                            <a href="#">Library</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">
                            <a href="#">Data</a>
                        </li>
                    </ol>
                </nav>
            </div>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb divide-dot">
                <li class="breadcrumb-item">
                    <a href="#">Home</a>
                </li>
                <li class="breadcrumb-item">
                    <a href="#">Library</a>
                </li>
                <li class="breadcrumb-item active" aria-current="page">
                    <a href="#">Data</a>
                </li>
            </ol>
        </nav>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <nav aria-label="breadcrumb">
            <ol className="breadcrumb divide-dot">
                <li className="breadcrumb-item">
                    <a href="#">Home</a>
                </li>
                <li className="breadcrumb-item">
                    <a href="#">Library</a>
                </li>
                <li className="breadcrumb-item active" aria-current="page">
                    <a href="#">Data</a>
                </li>
            </ol>
        </nav>
        ````
    </TabItem>
</Tabs>

### Accessibility Considerations

When implementing breadcrumbs, ensure they are accessible:

- Use `aria-label` on the `<nav>` element to describe its purpose.
- Mark the current page with `aria-current="page"` on the last breadcrumb item.

## Conclusion

The Breadcrumb component in the Arts Oven UI Component Library provides an intuitive way for users to navigate through your application. With various customization options and accessibility features, you can create effective breadcrumb navigation that enhances user experience. For more components and detailed documentation, refer to our main documentation page.