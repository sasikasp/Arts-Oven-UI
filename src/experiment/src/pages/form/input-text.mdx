---
layout: ../../layouts/docLayout.astro
title: Text Input
description: Text Input Element Documentation
---

import Card from '../../components/Card.astro'
import TabItem from '../../components/TabItem.astro'
import Tabs from '../../components/Tabs.astro'

The **Text Input** component is an essential part of any form, allowing users to enter and edit text.

## Text Input

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="flex justify-center">
                <input type="text" class="form-input" placeholder="First Name"/>
            </div>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <input type="text" class="form-input" placeholder="First Name"/>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <input type="text" className="form-input" placeholder="First Name"/>
        ````
    </TabItem>
</Tabs>

## Number Input

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="flex justify-center">
                <input type="number" class="form-input" placeholder="Quantity"/>
            </div>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <input type="number" class="form-input" placeholder="Quantity"/>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <input type="number" className="form-input" placeholder="Quantity"/>
        ````
    </TabItem>
</Tabs>

## Email Input

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="flex justify-center">
                <input type="email" class="form-input" placeholder="email@example.com"/>
            </div>
            <span class="block mt-2 text-sm pl-4">email@example.com</span>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <input type="email" class="form-input" placeholder="email@example.com"/>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <input type="email" className="form-input" placeholder="email@example.com"/>
        ````
    </TabItem>
</Tabs>

## URL Input

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="flex justify-center">
                <input type="url" class="form-input" placeholder="Website link"/>
            </div>
            <span class="block mt-2 text-sm pl-4">https://website.com</span>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <input type="url" class="form-input" placeholder="Website link"/>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <input type="url" className="form-input" placeholder="Website link"/>
        ````
    </TabItem>
</Tabs>

## Input Sizes

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="space-y-4">
                <input type="text" class="form-input input-sm" placeholder="First Name"/>
                <input type="text" class="form-input" placeholder="First Name"/>
                <input type="text" class="form-input input-lg" placeholder="First Name"/>
            </div>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <input type="text" class="form-input input-sm" placeholder="First Name"/>
        <input type="text" class="form-input" placeholder="First Name"/>
        <input type="text" class="form-input input-lg" placeholder="First Name"/>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <input type="text" className="form-input input-sm" placeholder="First Name"/>
        <input type="text" className="form-input" placeholder="First Name"/>
        <input type="text" className="form-input input-lg" placeholder="First Name"/>
        ````
    </TabItem>
</Tabs>

## Validation

<Tabs>
    <TabItem label="Preview">
        <Card>
            <div class="space-y-4">
                <input type="text" required novalidate class="form-input border-success" placeholder="First Name"/>
                <input type="text" class="form-input border-error" placeholder="First Name"/>
            </div>
        </Card>
    </TabItem>
    <TabItem label="HTML">
        ````xml
        <input type="text" class="form-input input-sm" placeholder="First Name"/>
        <input type="text" class="form-input" placeholder="First Name"/>
        <input type="text" class="form-input input-lg" placeholder="First Name"/>
        ````
    </TabItem>
    <TabItem label="JSX">
        ````xml
        <input type="text" className="form-input input-sm" placeholder="First Name"/>
        <input type="text" className="form-input" placeholder="First Name"/>
        <input type="text" className="form-input input-lg" placeholder="First Name"/>
        ````
    </TabItem>
</Tabs>


